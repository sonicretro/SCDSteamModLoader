...
namespace SCDSteamModManager
{
	public partial class MainForm : Form
	{
		...
		const string loaderinipath = "mods/SCDSteamModLoader.ini";
		...
		private void HandleUri(string uri)
		{
			...
			string name;
			string author;
			string[] split = uri.Substring("scdsteammm:".Length).Split(',');
			url = new Uri(split[0]);
		...
		private bool CheckForUpdates(bool force = false)
		{
			...
			if (!File.Exists("scdsteammlver.txt"))
			{
				return false;
			}
			...
			using (var wc = new WebClient())
			{
				try
				{
					string msg = wc.DownloadString("http://mm.reimuhakurei.net/toolchangelog.php?tool=scdsteamml&rev=" + File.ReadAllText("scdsteammlver.txt"));

					if (msg.Length > 0)
					{
						using (var dlg = new UpdateMessageDialog("SCD Steam", msg.Replace("\n", "\r\n")))
						{
							if (dlg.ShowDialog(this) == DialogResult.Yes)
							{
								...
								using (var dlg2 = new LoaderDownloadDialog("http://mm.reimuhakurei.net/misc/SCDSteamModLoader.7z", updatePath))
				...
				{
					MessageBox.Show(this, "Unable to retrieve update information.", "SCD Steam Mod Manager");
				}
			}
